...
+ With Routers: in this lab, there are three hosts (h1, h2, h3) and two OVS switches (s1, s2). H1 belongs to the LAN 192.168.1.0/24, h3 belongs to the LAN 192.168.2.0/24 and h3 belongs to both. Even though two switches are present, the real link between the two networks is h2: this host replaces the role of specific communication devices, which forward packets from one subnet to another. So, it represents the gateway of both networks.

+ Without Routers: in this lab, only the three hosts are present. As before, the two LANs are present and h2 belong to both of them, still working as the gateway.
